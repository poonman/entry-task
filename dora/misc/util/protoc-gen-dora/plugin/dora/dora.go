...
const (
	apiPkgPath     = "github.com/doraPlugin/go-doraPlugin/v2/api"
	contextPkgPath = "context"
	clientPkgPath  = "github.com/doraPlugin/go-doraPlugin/v2/client"
	serverPkgPath  = "github.com/doraPlugin/go-doraPlugin/v2/server"
)
...
func (g *doraPlugin) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
...
func (g *doraPlugin) generateEndpoint(servName string, method *pb.MethodDescriptorProto) {
	if method.Options == nil || !proto.HasExtension(method.Options, options.E_Http) {
		return
	}
	// http rules
...
func (g *doraPlugin) generateClientSignature(servName string, method *pb.MethodDescriptorProto) string {
...
func (g *doraPlugin) generateClientMethod(reqServ, servName, serviceDescVar string, method *pb.MethodDescriptorProto, descExpr string) {
...
func (g *doraPlugin) generateServerSignature(servName string, method *pb.MethodDescriptorProto) string {
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedClientName[methName] {
		methName += "_"
...
func (g *doraPlugin) generateServerMethod(servName string, method *pb.MethodDescriptorProto) string {
	methName := generator.CamelCase(method.GetName())
